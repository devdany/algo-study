...
export const bestAlbum = (genres, plays) => {
  ...
  const answer = Object.values(playPerGenre)
    .sort((a, b) => b.totalPlayCnt - a.totalPlayCnt)
    .reduce((acc, result) => acc.concat(result.playCnts.map(playCnt => playCnt.id)), [])
